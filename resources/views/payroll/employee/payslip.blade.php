		...
		<div class="row">
			<div class="col">
				<div class="card shadow">
					<div class="card-header">{{ __('Payslip') }}</div>
					<div class="card-body">
						<form method="POST" action="{{ route('payroll.employee.payslip.show') }}">
							@csrf
								...
								<tbody>
									@isset($payslips)
										@foreach($payslips as $payslip)
											<tr>
												<td>{{ $payslip->payrollcutoff->cut_off }}</td>
												<td>{{ date('Y-m-d', strtotime($payslip->created_at)) }}</td>
												<td></td>
												<td></td>
												<td>
