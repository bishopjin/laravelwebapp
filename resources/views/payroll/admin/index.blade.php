											...
											<tr>
												...
												<td>
													<a href="{{ route('payroll.admin.deduction.edit', ['id' => $deduc->id]) }}"
														class="btn btn-outline-success">{{ __('Edit') }}	
													</a>
												</td>
											</tr>
										@endforeach
									...
									@isset($users)
										@foreach($users as $user)
											<tr>
												...
												<td>{{ $user->salarygrade[0]->salary_grade ?? 'None' }}</td>
												<td>{{ $user->salarygrade->count() == 0 ? 'Not Registered' : 'Registered' }}</td>
												<td>
													<a href="{{ route('payroll.admin.user.edit', ['id' => $user->user_id]) }}" class="btn btn-outline-success">
														{{ $user->salarygrade->count() == 0 ? 'Register' : 'Edit' }}
													</a>
												</td>
											</tr>
										@endforeach
